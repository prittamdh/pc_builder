...
CRITICAL: "index" MUST equal the input model's number (1-based). One result per input model, exact count, any order (index is authoritative, not array position).
These are deep spec-sheet numbers that retailer titles almost never state - only fill a value if it is EXPLICITLY present in the listing text as a clearance (e.g. "supports GPU up to 400mm", "VGA Max 250mm"). Never use trained knowledge of the model: recalled clearances have proven wrong, and product pages are read separately. A bare size in a title such as "ATX 360mm" is radiator support, not a GPU length. Do NOT estimate from case size/form-factor category (e.g. "it's a mid-tower so probably ~350mm") - that is exactly the kind of plausible-sounding guess that must stay null instead. Expect most results to be null - that is the correct, honest outcome for this field.
notes: short reason if not high confidence, else null.
