"""
Per-listing Monitor identity extraction table.
Mirrors cpu_title_extraction.py: one row per listing, product_id + canonical_id
relation, brand/model_number. Physical display specs live separately in
monitor_specs, keyed by canonical_id (one row per unique model).
"""
...
class MonitorTitleExtraction(Base, TimestampMixin):
    __tablename__ = "monitor_title_extractions"
