...
- **src/api/schemas/product.py**: Pydantic response schemas `ProductOut` and `ProductListResponse` (including `p_category`).
...
- **src/api/routes/products.py**: Endpoints for product search, filtering by `p_category` and `category`, details, and price histories (`GET /api/v1/products`).
...
- **src/db/models/product.py**: Product model uniquely constrained on `(sid, pid)` with `category` and user-facing `p_category`.
...
- **src/db/repositories/product_repository.py**: CRUD operations for products using composite key `(sid, pid)` with `p_category` support.
...
- **src/services/search_service.py**: Service to persist search results, insert price history entries, and calculate `p_category`.
...
- **src/matching/category_classifier.py**: Categorization module classifying raw store categories into canonical `p_category` values.
...
- **scripts/populate_p_categories.py**: Classifies and populates `p_category` for all products in PostgreSQL.
...
- **scripts/update_granular_categories.py**: Enforces granular raw categories across all products in PostgreSQL.
