"""
Stage 1 - Groq LLM Monitor Identity Extraction (per listing, batched).
Extracts brand/model_number from each raw Monitor listing title, computes a
canonical key, upserts a thin canonical_parts row, and stamps products.canonical_id.
Physical specs (screen size/resolution/refresh rate/panel/response time) are NOT
extracted here - see extract_monitor_specs_groq.py (Stage 2), which runs once per
unique canonical_id instead of once per listing.
...
from services.groq_extraction_service import GroqExtractionService, GroqExtractionError, MONITOR_IDENTITY_BATCH_PROMPT
...
def extract_monitor_identity(reprocess_all: bool = False, limit: int | None = None, batch_size: int = 6, sleep_s: float = 0.0):
    with SessionLocal() as session, GroqExtractionService() as groq:
        ...
        stmt = select(Product).where(Product.p_category == "Monitor")
        ...
        total = len(products)
        print(f"Found {total} Monitor products needing identity extraction (reprocess_all={reprocess_all}).")
        ...
        for batch_start in range(0, total, batch_size):
            ...
            try:
                results = groq.extract_batch(MONITOR_IDENTITY_BATCH_PROMPT, titles)
                ...
                for product in batch:
                    ...
                    extraction = session.scalar(select(MonitorTitleExtraction).where(MonitorTitleExtraction.product_id == product.id))
                    if extraction is None:
                        extraction = MonitorTitleExtraction(product_id=product.id, raw_title=product.name)
            ...
            for product, result in zip(batch, results):
                ...
                canonical_id = make_canonical_key_string("monitor", key_dict)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Stage 1: extract Monitor identity (brand/model_number) via Groq LLM (batched).")
    parser.add_argument("--all", action="store_true", help="Reprocess all Monitor products, including ones with a canonical_id already.")
    ...
    extract_monitor_identity(reprocess_all=args.all, limit=args.limit, batch_size=args.batch_size, sleep_s=args.sleep)
