"""
Full batch scraper and normalizer script for Vedant Computers 10 Category Catalog targets.
Executes:
1. Scrapes all 10 category catalog endpoints up to 15 pages each (100 items per page limit).
...
def run_full_vedant_pipeline():
    print("=" * 80)
    print("STARTING FULL VEDANT COMPUTERS CATEGORY CATALOG SCRAPING PIPELINE")
    print("=" * 80)
    ...
    with SessionLocal() as session:
        ...
        if not store:
            print("[Error] Store Vedant Computers not found.")
            return

        targets = list(session.scalars(select(ScrapeTarget).where(ScrapeTarget.store_id == store.id)).all())
        print(f"Found {len(targets)} Category Catalog targets for Vedant Computers.")
    ...
    print("\n" + "=" * 80)
    print(f"PHASE 1 COMPLETE: Scraped & Saved {total_scraped_products} total products for Vedant Computers.")
    print("=" * 80)
    ...
    with SessionLocal() as session:
        ...
        normalized_count = norm_service.normalize_all_unclassified(limit=4000)
        print(f"  -> Normalized and populated category specs for {normalized_count} products.")

    elapsed = time.time() - start_time
    print("\n" + "=" * 80)
    print(f"VEDANT COMPUTERS FULL PIPELINE FINISHED IN {elapsed:.1f} SECONDS!")
    print("=" * 80)


if __name__ == "__main__":
    run_full_vedant_pipeline()
