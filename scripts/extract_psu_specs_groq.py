"""
Stage 2 - Groq LLM PSU Efficiency/Modularity Spec Extraction (per UNIQUE model, batched).
wattage/form_factor already come from psu_title_extractions (Stage 1, per-listing
identity) via CompatibilityEngine's merge fallback - this stage only fills
efficiency_rating/modularity, which are genuinely new fields.
Re-runnable: only processes canonical_ids that don't have a psu_specs row yet, unless --all.
"""
import argparse
import sys
import time
...
def clean_model_name(cp: CanonicalPart) -> str:
    kf = cp.key_fields or {}
    parts = [kf.get("brand", ""), kf.get("model_number", ""), kf.get("wattage", "")]
...
def extract_psu_specs(reprocess_all: bool = False, limit: int | None = None, batch_size: int = 6, sleep_s: float = 0.0):
    with SessionLocal() as session, GroqExtractionService(api_key=MISTRAL_API_KEY, model=MISTRAL_MODEL, api_url=MISTRAL_API_URL) as groq:
        ...
        print(f"Found {total} unique PSU models needing spec extraction (reprocess_all={reprocess_all}).")
        ...
        for batch_start in range(0, total, batch_size):
            ...
            try:
                results = groq.extract_batch(PSU_SPEC_BATCH_PROMPT, inputs)
            except GroqExtractionError as e:
                print(f"[batch @ {batch_start}] FAILED entire batch of {len(batch)}: {e}")
                for cp in batch:
                    existing = session.scalar(select(PSUSpecs).where(PSUSpecs.canonical_id == cp.canonical_id))
                    if existing is None:
                        existing = PSUSpecs(canonical_id=cp.canonical_id, status="pending")
            ...
            for cp, result in zip(batch, results):
                ...
                existing.efficiency_rating = as_str(parsed.get("efficiency_rating"))
                existing.modularity = as_str(parsed.get("modularity"))
            ...
            print(f"[{processed}/{total}] ok={ok_count} fail={fail_count} | last: '{clean_model_name(batch[-1])}' -> "
                  f"{results[-1]['parsed'].get('efficiency_rating')} {results[-1]['parsed'].get('modularity')} "
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Stage 2: extract PSU efficiency/modularity specs per unique model via Groq LLM (batched).")
    parser.add_argument("--all", action="store_true", help="Reprocess all canonical PSU models, including ones already extracted.")
    ...
    extract_psu_specs(reprocess_all=args.all, limit=args.limit, batch_size=args.batch_size, sleep_s=args.sleep)
