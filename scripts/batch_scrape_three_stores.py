"""
Full batch scraper and normalizer script for TPS Tech (sid=9), ModxComputers (sid=10), and TLG Gaming (sid=11).
Executes:
...
def run_batch_pipeline_for_three_stores():
    print("=" * 80)
    print("STARTING BATCH SCRAPING PIPELINE FOR TPS TECH, MODXCOMPUTERS & TLG GAMING")
    print("=" * 80)

    start_time = time.time()

    with SessionLocal() as session:
        stores = session.scalars(select(Store).where(Store.name.in_(["tpstech", "modxcomputers", "tlggaming"]))).all()
    ...
    print("\n" + "=" * 80)
    print(f"THREE STORES PIPELINE FINISHED IN {elapsed:.1f} SECONDS!")
    print("=" * 80)


if __name__ == "__main__":
    run_batch_pipeline_for_three_stores()
