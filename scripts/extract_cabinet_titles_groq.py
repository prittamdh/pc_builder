"""
Cabinet/Case Identity Extraction (per listing, batched, Groq LLM).
Extracts brand/model_number/color/form_factor from each raw Cabinet listing title.
Color is treated as a real identity field here (two colorways = different SKUs),
not noise to strip - this is the hardest category per the earlier difficulty ranking
(no standardized model-number convention, pure marketing names).
...
from services.groq_extraction_service import (
    GroqExtractionService, GroqExtractionError, CABINET_IDENTITY_BATCH_PROMPT,
    MISTRAL_API_KEY, MISTRAL_API_URL, MISTRAL_MODEL,
)


def extract_cabinet_identity(reprocess_all: bool = False, limit: int | None = None, batch_size: int = 6, sleep_s: float = 0.0):
    with SessionLocal() as session, GroqExtractionService(api_key=MISTRAL_API_KEY, model=MISTRAL_MODEL, api_url=MISTRAL_API_URL) as groq:
        print("=" * 80)
        print("GROQ LLM CABINET IDENTITY EXTRACTION (per listing, batched)")
        print("=" * 80)

        stmt = select(Product).where(Product.p_category == "Cabinet")
        ...
        total = len(products)
        print(f"Found {total} Cabinet products needing identity extraction (reprocess_all={reprocess_all}).")
        ...
        for batch_start in range(0, total, batch_size):
            ...
            try:
                results = groq.extract_batch(CABINET_IDENTITY_BATCH_PROMPT, titles)
                ...
                for product in batch:
                    ...
                    extraction = session.scalar(select(CabinetTitleExtraction).where(CabinetTitleExtraction.product_id == product.id))
                    if extraction is None:
                        extraction = CabinetTitleExtraction(product_id=product.id, raw_title=product.name)
            ...
            for product, result in zip(batch, results):
                ...
                canonical_id = make_canonical_key_string("case", key_dict)
                ...
                extraction.color = parsed.get("color")
                extraction.form_factor = parsed.get("form_factor")
            ...
            print(f"[{processed}/{total}] ok={ok_count} fail={fail_count} | last: '{batch[-1].name[:55]}' -> "
                  ...
                  f"{results[-1]['parsed'].get('color')} ({results[-1]['parsed'].get('confidence')})")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Extract Cabinet/Case identity via Groq LLM (batched).")
    parser.add_argument("--all", action="store_true", help="Reprocess all Cabinet products, including ones with a canonical_id already.")
    ...
    extract_cabinet_identity(reprocess_all=args.all, limit=args.limit, batch_size=args.batch_size, sleep_s=args.sleep)
