"""
Full batch scraper and normalizer script for PrimeABGB 10 Category Catalog targets.
Executes:
1. Scrapes all 10 category catalog endpoints up to 15 pages each (48 items per page).
...
def run_full_primeabgb_pipeline():
    print("=" * 80)
    print("STARTING FULL PRIMEABGB CATEGORY CATALOG SCRAPING PIPELINE")
    print("=" * 80)
    ...
    with SessionLocal() as session:
        ...
        print(f"Found {len(targets)} Category Catalog targets for PrimeABGB.")
    ...
    print("\n" + "=" * 80)
    print(f"PHASE 1 COMPLETE: Scraped & Saved {total_scraped_products} total products for PrimeABGB.")
    print("=" * 80)
    ...
    with SessionLocal() as session:
        ...
        normalized_count = norm_service.normalize_all_unclassified(limit=3000)
        print(f"  -> Normalized and populated category specs for {normalized_count} products.")

    elapsed = time.time() - start_time
    print("\n" + "=" * 80)
    print(f"PRIMEABGB FULL PIPELINE FINISHED IN {elapsed:.1f} SECONDS!")
    print("=" * 80)


if __name__ == "__main__":
    run_full_primeabgb_pipeline()
