"""
Storage Identity Extraction (per listing, batched, Groq LLM).
Extracts brand/model_number/capacity_gb/interface from each raw Storage listing title.
Identity + core identifying spec fields only - not deep specs (read/write speeds etc),
sourced elsewhere for now.
...
from services.groq_extraction_service import (
    GroqExtractionService, GroqExtractionError, STORAGE_IDENTITY_BATCH_PROMPT,
    MISTRAL_API_KEY, MISTRAL_API_URL, MISTRAL_MODEL,
)


def extract_storage_identity(reprocess_all: bool = False, limit: int | None = None, batch_size: int = 6, sleep_s: float = 0.0):
    with SessionLocal() as session, GroqExtractionService(api_key=MISTRAL_API_KEY, model=MISTRAL_MODEL, api_url=MISTRAL_API_URL) as groq:
        print("=" * 80)
        print("GROQ LLM STORAGE IDENTITY EXTRACTION (per listing, batched)")
        print("=" * 80)

        stmt = select(Product).where(Product.p_category == "Storage")
        ...
        total = len(products)
        print(f"Found {total} Storage products needing identity extraction (reprocess_all={reprocess_all}).")
        ...
        for batch_start in range(0, total, batch_size):
            ...
            try:
                results = groq.extract_batch(STORAGE_IDENTITY_BATCH_PROMPT, titles)
                ...
                for product in batch:
                    ...
                    extraction = session.scalar(select(StorageTitleExtraction).where(StorageTitleExtraction.product_id == product.id))
                    if extraction is None:
                        extraction = StorageTitleExtraction(product_id=product.id, raw_title=product.name)
            ...
            for product, result in zip(batch, results):
                ...
                key_dict = {
                    ...
                    "series": parsed.get("model_number") or "",
                    "capacity": f"{parsed.get('capacity_gb')}gb" if parsed.get("capacity_gb") else "",
                    "interface": parsed.get("interface") or "",
                }
                key_dict = disambiguate_failed_key(key_dict, product.id)
                canonical_id = make_canonical_key_string("storage", key_dict)
                ...
                extraction.capacity_gb = parsed.get("capacity_gb")
                extraction.interface = parsed.get("interface")
            ...
            print(f"[{processed}/{total}] ok={ok_count} fail={fail_count} | last: '{batch[-1].name[:55]}' -> "
                  ...
                  f"{results[-1]['parsed'].get('capacity_gb')}GB ({results[-1]['parsed'].get('confidence')})")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Extract Storage identity via Groq LLM (batched).")
    parser.add_argument("--all", action="store_true", help="Reprocess all Storage products, including ones with a canonical_id already.")
    ...
    extract_storage_identity(reprocess_all=args.all, limit=args.limit, batch_size=args.batch_size, sleep_s=args.sleep)
